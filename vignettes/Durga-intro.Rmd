...
The default behaviour of `DurgaDiff` is to calculate differences between all pairs of groups. Due to the difficulty of laying out multiple effect sizes, `DurgaPlot` treats the first group as a control, and shows the differences between all subsequent groups and the control.  The first example plot below demonstrates the problems caused by drawing all effect sizes; two effect sizes appear at the same x-location and their labels are overwritten. To change the default behaviour, specify the `contrasts` argument to either `DurgaDiff` or `DurgaPlot` (or see [Confidence brackets](#confidence-brackets) for an alternative technique for displaying multiple effect sizes). Contrasts are specified as "group - group, group - group", so in order to show two of the three possible effect sizes from the `iris` data set, you could specify `contrasts = c("virginica - setosa", "versicolor - setosa")`, as shown below.

Effect sizes are plotted directly below the first group in the contrast, so another possible method for showing all effect sizes with three groups is to display one effect size under each group. For the `iris` data set, this could be achieved with `contrasts = c("virginica - setosa", "setosa - versicolor", "versicolor - virginica")`. Each effect size is clearly visible, however, it is necessary to read the labels underneath an effect size to know the groups to which it applies.

```{r fig.height=4, fig.width=8}
par(mfrow = c(1, 2))

di <- DurgaDiff(iris, "Sepal.Length", "Species")
DurgaPlot(di, contrasts = "*", main = "All effect sizes")

DurgaPlot(di, contrasts = c("virginica - setosa", "versicolor - setosa"), main = "Selected effect sizes")
```
...
`group.dx` shifts all components equally, however, each component can be shifted independently. For example, to shift all _violins_ left and all _points_ right, specify `violin.dx = -0.12, points.dx = 0.1`. 
...
Colours can be specified for each group (`group.colour`; demonstrated above), or separately for individual components. The _master_ argument for each component type controls whether the component is displayed and optionally its colour. So, to display blue violins, specify `violin = "blue"`. Colours can be specified as the name of an [RColorBrewer](https://r-graph-gallery.com/38-rcolorbrewers-palettes.html) palette (e.g. `group.colour = "Dark2"`), or as one or more R colours. 
...
# Draw the brackets
DurgaBrackets(p, labels = "diff CI")
```
...
Durga provides a fixed set of effect type statistics that can be used, however sometimes a different statistic is required. For example, we may wish to compare measurements of some organ or structure to know whether they have increased or decreased in size, in which case we may decide to use log~2~ ratio as our statistic. log~2~ ratio has a straightforward interpretation: 0 means no difference, 1 means double the size, 2 means four-times the size, -1 means half the size, and so on.
...
# Reduce text size
par(cex = 0.7)
DurgaPlot(d, box = TRUE, group.dx = c(0.15, -0.15), paired = FALSE, points = FALSE, xlim = c(1, 6), 
          # Hide the boxplot median line and make the boxes narrow 
          #   (box.pars = list(medcol = NA, boxwex = 0.4))
          box.pars = list(medcol = NA, boxwex = 0.4), 
